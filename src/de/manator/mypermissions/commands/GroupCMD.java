				...
				if (args.length == 2) {
					...
					if (args[0].equalsIgnoreCase("add")) {
						...
						if(gh.setPrefix(gh.getGroup(args[1]), args[2].replace('&', '§'))) {
							CMD.sendMessage(p, "§aThe prefix of the group §6" + args[1] + "§a was set to §r + " + gh.getGroup(args[1]).getPrefix());
						} else {
						...
						if(gh.setSuffix(gh.getGroup(args[1]), args[2].replace('&', '§'))) {
							CMD.sendMessage(p, "§aThe suffix of the group §6" + args[1] + "§a was set to §r + " + gh.getGroup(args[1]).getSuffix());
						} else {
